    ...
    def bound_backward(self, last_lA, last_uA, x, start_node=None,
        ...
        if True:
            max_upper, max_upper_index = F.max_pool2d(
                x.upper.detach(), self.kernel_size, self.stride, self.padding,
                return_indices=True, ceil_mode=self.ceil_mode)
            max_lower, max_lower_index = F.max_pool2d(
                x.lower.detach(), self.kernel_size, self.stride, self.padding,
                return_indices=True, ceil_mode=self.ceil_mode)
            middle=(x.upper.detach()+x.lower.detach())/2
            #the 
            ...
            if paddings == (0,0,0,0):
                delete_upper = torch.scatter(
                    torch.flatten(x.upper.detach(), -2), -1,
                    torch.flatten(max_upper_index, -2), -torch.inf).view(upper_d.shape)
            else:

                delete_upper = torch.scatter(
                    torch.flatten(F.pad(x.upper.detach(), paddings), -2), -1,
            ...
            max_upper2, max_upper2_index = F.max_pool2d(
                delete_upper, self.kernel_size, self.stride, self.padding,
                return_indices=True, ceil_mode=self.ceil_mode)

            if paddings == (0,0,0,0):
                delete_upper2 = torch.scatter(
                    torch.flatten(delete_upper, -2), -1,
                    torch.flatten(max_upper2_index.detach(), -2), -torch.inf).view(upper_d.shape)
            else:

                delete_upper2 = torch.scatter(
                    torch.flatten(F.pad(delete_upper, paddings), -2), -1,
                    torch.flatten(max_upper2_index.detach(), -2),
                    -torch.inf).view(upper_d.shape)
            max_upper3, max_upper3_index = F.max_pool2d(
                delete_upper2, self.kernel_size, self.stride, self.padding,
                return_indices=True, ceil_mode=self.ceil_mode)
            
            if paddings == (0,0,0,0):
                max_upper_lower_index = torch.scatter(
                    torch.flatten(torch.full(shape,-torch.inf, device=x.device), -2), -1,
                    torch.flatten(max_upper_index, -2), 0.0).view(upper_d.shape)
                max_upper2_lower_index = torch.scatter(
                    torch.flatten(torch.full(shape,-torch.inf, device=x.device), -2), -1,
                    torch.flatten(max_upper2_index, -2), 0.0).view(upper_d.shape)

            else:

                max_upper_lower_index = torch.scatter(
                    torch.flatten(F.pad(torch.full(shape,-torch.inf, device=x.device), paddings,values=-torch.inf), -2), -1,
                    torch.flatten(max_upper_index, -2),
                    0.0).view(upper_d.shape)
                max_upper2_lower_index = torch.scatter(
                    torch.flatten(F.pad(torch.full(shape,-torch.inf, device=x.device), paddings,values=-torch.inf), -2), -1,
                    torch.flatten(max_upper2_index, -2),
                    0.0).view(upper_d.shape)


            max_upper_lower, max_upper_lower_index1 = F.max_pool2d(
                max_upper_lower_index.detach()+x.lower.detach(), self.kernel_size, self.stride, self.padding,
                return_indices=True, ceil_mode=self.ceil_mode)
            max_upper2_lower, max_upper2_lower_index1 = F.max_pool2d(
                max_upper2_lower_index.detach()+x.lower.detach(), self.kernel_size, self.stride, self.padding,
                return_indices=True, ceil_mode=self.ceil_mode)

            assert torch.all(max_upper_index==max_upper_lower_index1)
            assert torch.all(max_upper2_index==max_upper2_lower_index1)

            assert torch.all(max_upper_lower<=max_upper)
            #######maxlin+tilin#######
            max_negative_lower, lower_condition_index = F.max_pool2d(
                0.0-x.lower.detach(), self.kernel_size, self.stride, self.padding,
                return_indices=True, ceil_mode=self.ceil_mode)
            lower_condition= (max_negative_lower<=0)
            upper_condition=(max_upper<=0)

  
            ti_condition=lower_condition&upper_condition
       
            
            
            values = torch.zeros_like(max_lower, device=x.device)#for a_i
            upper_case1 = torch.zeros_like(max_lower, device=x.device)
            upper_case2 = torch.zeros_like(max_lower, device=x.device)
            upper_case3 = torch.zeros_like(max_lower, device=x.device)
            upper_case4 = torch.zeros_like(max_lower, device=x.device)
            max_i=max_upper_lower.clone()
            max_j=max_upper2_lower.clone()
            max_i[max_upper_lower<max_upper2_lower]=max_j[max_upper_lower<max_upper2_lower]
            #case1:
            upper_case1[max_upper_lower>=max_lower-1e-6]=1.0
            upper_case1[max_upper_lower>=max_upper2]=upper_case1[max_upper_lower>=max_upper2]*2
            #case2:
            upper_case2[max_upper_lower>=max_lower]=1.0
            upper_case2[max_upper2>max_upper_lower]=upper_case2[max_upper2>max_upper_lower]*2
            upper_case2[max_upper_lower>=max_upper3]=upper_case2[max_upper_lower>=max_upper3]*2
            
            #case3:
            upper_case3[max_upper2_lower>=max_lower]=1.0
            upper_case3[max_upper_lower<max_lower]=upper_case3[max_upper_lower<max_lower]*2
            upper_case3[max_upper2>max_upper2_lower]=upper_case3[max_upper2>max_upper2_lower]*2
            upper_case3[max_upper2_lower>=max_upper3]=upper_case3[max_upper2_lower>=max_upper3]*2
            #case4:
            upper_case4[max_i<max_upper3]=1.0
            #case 1
            values[upper_case1>1.5] = 1.0
            #case 2
            values[upper_case2>3.5] = 1.0
            #case3
            temp=(max_upper-max_upper2_lower)/(max_upper-max_upper_lower)
            values = torch.where(upper_case3>5.5, temp, values)
            #case4
            temp2=(max_upper-max_upper3)/(max_upper-max_upper_lower)
            values = torch.where(max_i < max_upper3, temp2, values)
            
            #maxlin-case2
            temp=(max_upper-max_upper2)/(max_upper-max_upper_lower)
            max_condition=~ti_condition
            values=torch.where(max_condition,temp,values)
            #case-1
            max_condition2=(~ti_condition)&((max_lower<=max_upper_lower)&(max_upper_lower>=max_upper2))
            values=torch.where(max_condition2,1.0,values)
            
            upper_d_temp = torch.scatter(
                torch.flatten(torch.zeros(shape, device=x.device), -2), -1,
                torch.flatten(max_upper_index, -2),
                torch.flatten(values, -2)).view(upper_d.shape)
            
            #for a_j 
            upper_d2 = torch.zeros(shape, device=x.device)
            values2 = torch.zeros_like(max_lower, device=x.device)
            #case2
            temp3=(max_upper2-max_upper_lower)/(max_upper2-max_upper2_lower)
            values2 = torch.where(upper_case2>3.5, temp3, values2)
            #case3
            values2[upper_case3>5.5]=1.0
            #case4
            temp2=(max_upper2-max_upper3)/(max_upper2-max_upper2_lower)
            values2 = torch.where(max_i < max_upper3, temp2, values2)
            #case1
            values2[upper_case1>1.5]=0.0

            #maxlin
            values2=torch.where(max_condition,0.0,values2)
            
            upper_d2 = torch.scatter(
                torch.flatten(torch.zeros(shape, device=x.device), -2), -1,
                torch.flatten(max_upper2_index, -2),
                torch.flatten(values2, -2)).view(upper_d.shape)            
            upper_d=upper_d_temp+upper_d2

            #begin to compute the bias of upper linear bound
            #case2
            temp_b=-1.0*max_upper_lower - (max_upper2-max_upper_lower)/(max_upper2-max_upper2_lower)*max_upper2_lower+max_lower
            upper_b = torch.where(upper_case2>3.5, temp_b, upper_b)
            #case3
            temp_b_=-(max_upper-max_upper2_lower)/(max_upper-max_upper_lower)*max_upper_lower - 1.0*max_upper2_lower+max_lower
            upper_b = torch.where(upper_case3>5.5, temp_b_, upper_b)
            #case4
            temp_b2=-(max_upper-max_upper3)/(max_upper-max_upper_lower)*max_upper_lower - (max_upper2-max_upper3)/(max_upper2-max_upper2_lower)*max_upper2_lower+max_upper3
            upper_b = torch.where(max_i < max_upper3, temp_b2, upper_b)
            #case1 
            upper_b[upper_case1>1.5]=0.0
            
            
            #maxlin-case2
            temp=-(max_upper-max_upper2)/(max_upper-max_upper_lower)*max_upper_lower+max_upper2#-max_lower
            upper_b=torch.where(max_condition,temp,upper_b)
            #case-1
            upper_b=torch.where(max_condition2,0.0,upper_b)
