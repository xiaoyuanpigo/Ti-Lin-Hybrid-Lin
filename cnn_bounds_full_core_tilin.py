...
Main computing file for networks with just convolution and pooling layers
...
@njit
def upper_bound_conv(A, B, pad, stride, W, b, inner_pad, inner_stride, inner_shape, LB, UB, f_mid):
    A_new = np.zeros((A.shape[0], A.shape[1], A.shape[2], inner_stride[0]*(A.shape[3]-1)+W.shape[1], inner_stride[1]*(A.shape[4]-1)+W.shape[2], W.shape[3]), dtype=np.float32)
    B_new = np.zeros(B.shape, dtype=np.float32)
    A_plus = np.maximum(A, 0)
    A_minus = np.minimum(A, 0)
    alpha_u, alpha_l, beta_u, beta_l = linear_bounds(LB, UB,f_mid)
    assert A.shape[5] == W.shape[0]
...
@njit
def lower_bound_conv(A, B, pad, stride, W, b, inner_pad, inner_stride, inner_shape, LB, UB, f_mid):
    A_new = np.zeros((A.shape[0], A.shape[1], A.shape[2], inner_stride[0]*(A.shape[3]-1)+W.shape[1], inner_stride[1]*(A.shape[4]-1)+W.shape[2], W.shape[3]), dtype=np.float32)
    B_new = np.zeros(B.shape, dtype=np.float32)
    A_plus = np.maximum(A, 0)
    A_minus = np.minimum(A, 0)
    ...
    for x in range(alpha_u.shape[2]):
        for y in range(alpha_u.shape[3]):
            for r in range(alpha_u.shape[4]):
                ...
                cropped_mid = 0.5*(LB[s_h*x-p_hl:pool_size[0]+s_h*x-p_hl, s_w*y-p_wl:pool_size[1]+s_w*y-p_wl,r]+LB[s_h*x-p_hl:pool_size[0]+s_h*x-p_hl, s_w*y-p_wl:pool_size[1]+s_w*y-p_wl,r])
                ...
                m_s = np.zeros(len(idx[0]), dtype=np.float32)

                for i in range(len(idx[0])):
                    l_s[i] = cropped_LB[idx[0][i],idx[1][i]]
                    u_s[i] = cropped_UB[idx[0][i],idx[1][i]]
                    m_s[i] = cropped_mid[idx[0][i],idx[1][i]]
                ...
                if u_s[ju]<=l_s[iu] and max_LB<=l_s[iu]:#case1
                    weights[iu] = 1
                    intercept = l_s[iu] # 
                elif u_s[ju]>l_s[iu] and max_LB<=l_s[iu] and u_s[ku]<=l_s[iu]:#case2
                    weights[ju] = (u_s[ju]- l_s[iu]) / (u_s[ju] - l_s[ju])
                    weights[iu] = 1
                    intercept = l_s[iu]
                elif u_s[ju]>l_s[ju] and max_LB<=l_s[ju] and u_s[ku]<=l_s[ju] and l_s[iu]<l_s[ju]:#case3
                    weights[iu] = (u_s[iu]- l_s[ju]) / (u_s[iu] - l_s[iu])
                    weights[ju] = 1
                    intercept = l_s[ju]
                else:#case4
                    weights[iu] = (u_s[iu] - u_s[ku]) / (u_s[iu] - l_s[iu])
                    weights[ju] = (u_s[ju] - u_s[ku]) / (u_s[ju] - l_s[ju])
                    intercept = u_s[ku]
...
def run(file_name, n_samples, p_n, q_n, activation = 'relu', cifar=False, tinyimagenet=False):
    ...
    for i in range(len(inputs)):
        ...
        print("[L1] method = MaxLin-{}, model = {}, image no = {}, true_id = {}, target_label = {}, true_label = {}, norm = {}, robustness = {:.5f}".format(activation,file_name, i, true_ids[i],target_label,predict_label,str_p_n,np.exp(log_eps_min)))
        summation += np.exp(log_eps_min)
    K.clear_session()
    
    eps_avg = summation/len(inputs)
    total_time = (time.time()-start_time)/len(inputs)
    print("[L0] method = MaxLin-{}, model = {}, total images = {}, norm = {}, avg robustness = {:.5f}, avg runtime = {:.2f}".format(activation,file_name,len(inputs),str_p_n,eps_avg,total_time))
