...
class BoundMaxPool(BoundOptimizableActivation):
    ...
    def bound_backward(self, last_lA, last_uA, x, start_node=None,
        ...
        # self.padding is a tuple of two elements: (height dimension padding, width dimension padding).!!!!!this matters--xy
        ...
        if True:
            ...
            if paddings == (0,0,0,0):
                delete_upper2 = torch.scatter(
                    ...
                    torch.flatten(max_upper2_index, -2), -torch.inf).view(upper_d.shape)
            else:
            ...
            max_upper_lower, max_upper_lower_index1 = F.max_pool2d(
                max_upper_lower_index.clone()+x.lower.detach(), self.kernel_size, self.stride, self.padding,
                return_indices=True, ceil_mode=self.ceil_mode)
            max_upper2_lower, max_upper2_lower_index1 = F.max_pool2d(
                max_upper2_lower_index.clone()+x.lower.detach(), self.kernel_size, self.stride, self.padding,
                return_indices=True, ceil_mode=self.ceil_mode)
            ...
            #case2
            b=-1.0*max_upper_lower - (max_upper2-max_upper_lower)/(max_upper2-max_upper2_lower)*max_upper2_lower+max_lower
            upper_b = torch.where(upper_case2>3.5, b, upper_b)
            #case3
            b_=-(max_upper-max_upper2_lower)/(max_upper-max_upper_lower)*max_upper_lower - 1.0*max_upper2_lower+max_lower
            upper_b = torch.where(upper_case3>5.5, b_, upper_b)
            #case4
            b2=-(max_upper-max_upper3)/(max_upper-max_upper_lower)*max_upper_lower - (max_upper2-max_upper3)/(max_upper2-max_upper2_lower)*max_upper2_lower+max_upper3
            upper_b = torch.where(max_i < max_upper3, b2, upper_b)
            #case1 
            upper_b[upper_case1>1.5]=0.0

            # del max_upper2_lower_index, max_upper_lower_index,max_upper3_lower_index, max_upper_index,max_upper2_index,max_upper3_index
    ...
    def bound_forward(self, dim_in, x):
        ...
        # print('lw, lb, uw, ub!bf',lw, lb, uw, ub)
        # print("------------------bound_forward------------")
    ...
    def bound_relax(self, x, init=False, dim_opt=None):
        ...
        # print('lower_d, lower_b, upper_d, upper_b',lower_d, lower_b, upper_d, upper_b)
        # print("------------bound_relax------------------------")
    ...
    def build_solver(self, *v, model, C=None, model_type="mip", solver_pkg="gurobi"):
        ...
        assert this_layer_shape[2] ==  ((2 * self.padding[0] + pre_layer_shape[2] - (self.stride[0] - 1))//self.stride[0]+1) or this_layer_shape[2] ==  ((2 * self.padding[0] + pre_layer_shape[2] - (self.stride[0] - 1))//self.stride[0])
